...
BOT_TOKEN = "8716418300:AAHgrLACti5PaPT6oqsuqzvGRVyv8Vyl2Ms"
OPENAI_API_KEY = "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"
...
TEXTS = {
    "ru": {
        ...
        "model_changed": "Модель обновлена ✅",
        ...
        "btn_model_main": "gpt-4.1-mini",
        "btn_model_alt": "gpt-4.1",
    },
    "en": {
        "welcome": "Hi, {name} 👋\n\nI am a multifunctional AI bot in Telegram.\n\nI can:\n• answer like ChatGPT\n• remember conversation\n• change response style\n• switch model\n• export history\n• clear memory\n\nChoose an action below or just send a message.",
        "menu": "Main menu",
...
STYLE_PROMPTS = {
    "balanced": {
        "ru": "Отвечай полезно, ясно и по делу. Будь естественным, умным и структурным. Не растягивай ответ без причины.",
        "en": "Answer helpfully, clearly, and directly. Be natural, smart, and structured. Do not be verbose without reason.",
    },
    "precise": {
        "ru": "Отвечай максимально точно, строго и профессионально. Убирай воду. Давай чёткие шаги и конкретику.",
        "en": "Answer as precisely, rigorously, and professionally as possible. Avoid fluff. Give clear steps and specifics.",
    },
    "friendly": {
        "ru": "Отвечай дружелюбно, тепло и понятно. Объясняй простыми словами, но сохраняй пользу и качество.",
        "en": "Answer in a friendly, warm, and clear way. Explain simply while keeping the answer useful and high-quality.",
    },
    "short": {
        "ru": "Отвечай кратко, но полезно. Только главное. Без лишних вступлений.",
        "en": "Answer briefly but usefully. Only the essentials. No unnecessary introductions.",
    },
}
...
class Database:
    ...
    def upsert_user(self, user_id: int, username: str, full_name: str):
        ...
        if exists:
            ...
            cur.execute(
                """
                INSERT INTO users (
                    user_id, username, full_name, language, model, style_key,
                    custom_style, chat_enabled, awaiting_custom_style, created_at, updated_at
                ) VALUES (?, ?, ?, 'ru', ?, 'balanced', '', 1, 0, ?, ?)
                """,
                (user_id, username, full_name, OPENAI_MODEL, now, now),
            )
        self.conn.commit()
    ...
    def stats(self):
        ...
        cur.execute("SELECT COUNT(*) AS c FROM users WHERE chat_enabled=1")
        active = cur.fetchone()["c"]
        return users, messages, active
...
def main_menu(user_id: int):
    ...
    buttons = [
        ...
        [
            ...
            InlineKeyboardButton(text=t(lang, "btn_model"), callback_data="settings:model"),
        ],
        [
...
def model_menu(user_id: int):
    lang = get_lang(user_id)
    buttons = [
        [InlineKeyboardButton(text=t(lang, "btn_model_main"), callback_data="model:gpt-4.1-mini")],
        [InlineKeyboardButton(text=t(lang, "btn_model_alt"), callback_data="model:gpt-4.1")],
        [InlineKeyboardButton(text=t(lang, "btn_back"), callback_data="back:menu")],
    ]
    return InlineKeyboardMarkup(inline_keyboard=buttons)
...
def build_system_prompt(user: dict):
    ...
    base = [
        ...
        "Do not mention hidden policies or internal mechanics.",
...
async def ask_ai(user: dict, user_text: str):
    ...
    input_messages = [{"role": "system", "content": build_system_prompt(user)}]
    for item in history:
...
async def stats_handler(message: Message):
    ...
    if ADMIN_IDS and message.from_user.id not in ADMIN_IDS:
        return
    user = db.get_user(message.from_user.id)
    users, messages_count, active = db.stats()
    await message.answer(
        t(user["language"], "stats", users=users, messages=messages_count, active=active),
        reply_markup=main_menu(message.from_user.id),
    )
...
async def settings_model_handler(callback: CallbackQuery):
    if not callback.from_user or not callback.message:
        return
    user = db.get_user(callback.from_user.id)
    await callback.message.edit_text(t(user["language"], "choose_model"), reply_markup=model_menu(callback.from_user.id))
    await callback.answer()
...
async def model_change_handler(callback: CallbackQuery):
    if not callback.from_user or not callback.message:
        return
    model = callback.data.split(":", 1)[1]
    db.update_user_field(callback.from_user.id, "model", model)
    user = db.get_user(callback.from_user.id)
    await callback.message.edit_text(t(user["language"], "model_changed"), reply_markup=main_menu(callback.from_user.id))
    await callback.answer()
...
async def text_handler(message: Message, bot: Bot):
    ...
    try:
        ...
        logging.exception("AI response error")
        with suppress(Exception):
...
async def main():
    if BOT_TOKEN.startswith("PASTE_") or OPENAI_API_KEY.startswith("PASTE_"):
        raise RuntimeError("Set BOT_TOKEN and OPENAI_API_KEY inside the file before launch.")
    bot = Bot(
        token=BOT_TOKEN,
